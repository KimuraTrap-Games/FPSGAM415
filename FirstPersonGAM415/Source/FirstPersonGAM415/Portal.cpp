...
{
	AFirstPersonGAM415Character* playerChar = Cast<AFirstPersonGAM415Character>(OtherActor);
...
void APortal::SetBool(AFirstPersonGAM415Character* playerChar)
{
	if (playerChar)
	{
...
{
	FVector Location = GetActorLocation() - OtherPortal->GetActorLocation();
	...
	FVector CombindedLocation = camLocation + Location;

	sceneCapture->SetWorldLocationAndRotation(CombindedLocation, camRotation);
}
